...
public class Recv2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //设置消费者预取的消息数量为1（即：每次只取1条消息，消费完，再取下一条）
        channel.basicQos(1);
        //创建队列，声明创建一个队列，如果队列已经存在，则使用这个队列
        channel.queueDeclare(QUEUE_NAME,false,false,false,null);

        //绑定队列到交换机上
        channel.queueBind(QUEUE_NAME,EXCHANGE_NAME,"info");//注意：参数3表示routing-key
        channel.queueBind(QUEUE_NAME,EXCHANGE_NAME,"error");//注意：参数3表示routing-key
        channel.queueBind(QUEUE_NAME,EXCHANGE_NAME,"warn");//注意：参数3表示routing-key
